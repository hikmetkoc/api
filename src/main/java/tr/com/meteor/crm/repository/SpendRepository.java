...
public interface SpendRepository extends GenericIdNameAuditingEntityRepository<Spend, UUID> {
    ...
    @Transactional
    @Modifying
    @Query(value = "INSERT INTO Spend (id, created_by_id, status_id, paymentorder_id, amount, payment_status, lock, finance_id, created_date, maturity_date, payment_num, pay_tl, owner_id, spend_date, customer_id, odeme_yapan_sirket) VALUES (:id, :owner, :status, :paymentorder, :amount, :paymentStatus, :lock, :finance, :createdDate, :maturityDate, :paymentNum, :payTl, :owner, :createdDate, :customer, :odemeYapanSirket)", nativeQuery = true)
    void insertSpend(@Param("id") UUID id, @Param("owner") Long owner, @Param("status") String status, @Param("paymentorder") UUID paymentorder, @Param("amount") BigDecimal amount, @Param("paymentStatus") String paymentStatus, @Param("lock") Boolean lock, @Param("finance") Long finance, @Param("createdDate") Instant createdDate, @Param("maturityDate") Instant maturityDate, @Param("paymentNum") String paymentNum, @Param("payTl") BigDecimal payTl, @Param("customer") UUID customer, @Param("odemeYapanSirket") String odemeYapanSirket);
}
